    ...
    \usepackage{listings}
% Configuración global de listings - Estilo unificado para todo el documento
\lstset{
...
% Estilo para fragmentos PDDL (mismo estilo visual que Python)
...
% Estilo para anexos (mismo estilo visual, solo añade numeración)
\lstdefinestyle{anexostyle}{
    ...
    \begin{lstlisting}[basicstyle=\scriptsize\ttfamily,breaklines=true,frame=single,xleftmargin=0.2cm,xrightmargin=0.2cm,backgroundcolor=\color{gray!5}]
    ...
    \subsection*{Anexo C: Validación de Ontología}

    Este anexo presenta los resultados de la validación completa de la ontología del dominio, verificando consistencia conceptual, mapeo a PDDL y cobertura de relaciones.

    \subsubsection*{C.1: Resumen de Validación}

    \begin{table}[H]
    \caption{Resumen cuantitativo de la ontología validada}
    ...
    \midrule
    Conceptos definidos & 15 \\
    Relaciones semánticas & 15 \\
    Atributos totales & 47 \\
    Instancias concretas & 59 \\
    Tipos PDDL mapeados & 15 \\
    Predicados PDDL & 15 \\
    Relaciones estáticas & 4 \\
    Relaciones dinámicas & 11 \\
    Atributos mutables & 28 \\
    Atributos inmutables & 19 \\
    \bottomrule
    \end{tabular}
    \end{table}

    \subsubsection*{C.2: Mapeo Completo Conceptos → Tipos PDDL}

    La Tabla~\ref{tab:mapeo_pddl} muestra el mapeo completo y validado entre los conceptos de la ontología y su representación en PDDL. Cada concepto tiene un tipo PDDL correspondiente que preserva la jerarquía semántica.

    \begin{table}[H]
    \caption{Mapeo de conceptos de ontología a tipos PDDL}
    \label{tab:mapeo_pddl}
    \centering
    \scriptsize
    \begin{tabular}{@{}llll@{}}
    \toprule
    \textbf{ID} & \textbf{Concepto} & \textbf{Tipo PDDL} & \textbf{Validación} \\
    \midrule
    C01 & Locacion & locacion & \checkmark \\
    C02 & UnidadMovil & unidad\_movil & \checkmark \\
    C03 & PersonalRescate & personal\_rescate & \checkmark \\
    C04 & PersonalConstruccion & personal\_construccion & \checkmark \\
    C05 & Victima & victima (3 subtipos) & \checkmark \\
    C06 & Infraestructura & infraestructura & \checkmark \\
    C07 & Recurso & recurso & \checkmark \\
    C08 & EstadoSalud & tipos enumerados & \checkmark \\
    C09 & EstadoInfraestructura & predicados de estado & \checkmark \\
    C10 & Capacidad & fluent numérico & \checkmark \\
    C11 & Conductor & conductor & \checkmark \\
    C12 & ZonaEstacionamiento & predicado sobre locacion & \checkmark \\
    C13 & Base & predicado sobre locacion & \checkmark \\
    C14 & ZonaDesastre & predicado sobre locacion & \checkmark \\
    C15 & Tarea & predicado tarea\_completada & \checkmark \\
    \bottomrule
    \end{tabular}
    \end{table}

    \subsubsection*{C.3: Tabla de Cobertura de Relaciones}

    La Tabla~\ref{tab:cobertura_relaciones} analiza cada relación semántica, su cardinalidad, propiedades matemáticas y su implementación en PDDL.

    \begin{table}[H]
    \caption{Análisis de cobertura de relaciones semánticas}
    \label{tab:cobertura_relaciones}
    \centering
    \scriptsize
    \begin{tabular}{@{}lllllll@{}}
    \toprule
    \textbf{ID} & \textbf{Relación} & \textbf{Card.} & \textbf{Func.} & \textbf{Sim.} & \textbf{Dinámica} & \textbf{PDDL} \\
    \midrule
    R01 & en & N:1 & Sí & No & Sí & (en ?e ?l) \\
    R02 & conecta & 1:2 & No & Sí & Sí & (conecta ?i ?l1 ?l2) \\
    R03 & transporta & 1:N & No & No & Sí & (dentro ?p ?u) \\
    R04 & capacidad & 1:1 & Sí & No & Sí & (capacidad\_disponible ?u ?n) \\
    R05 & estabiliza & 1:N & No & No & Sí & (estabilizado ?v) \\
    R06 & construye & N:1 & No & No & Sí & (construido ?i) \\
    R07 & requiere & 1:N & No & No & No & precondiciones \\
    R08 & adyacente & N:N & No & Sí & No & (adyacente ?l1 ?l2) \\
    R09 & operativo & 1:1 & Sí & No & Sí & (operativo ?i) \\
    R10 & rescatado & 1:1 & Sí & No & Sí & (rescatado ?v) \\
    R11 & puede\_avanzar\_a & 1:1 & Sí & No & Sí & (puede\_avanzar\_a ?l) \\
    R12 & asignado\_a\_tarea & 1:1 & Sí & No & Sí & (asignado\_a\_tarea ?o ?l) \\
    R13 & disponible & 1:1 & Sí & No & Sí & (disponible ?p) \\
    R14 & tarea\_completada & 1:1 & Sí & No & Sí & (tarea\_completada ?i) \\
    R15 & base & 1:1 & Sí & No & No & (base ?l) \\
    \bottomrule
    \multicolumn{7}{@{}l@{}}{\scriptsize Func. = Funcional; Sim. = Simétrica} \\
    \end{tabular}
    \end{table}

    \subsubsection*{C.4: Verificación de Consistencia Conceptual}

    \textbf{Jerarquía de tipos validada}: Se verificó la correcta herencia de tipos en PDDL. El tipo \texttt{persona} se subdivide en 4 categorías mutuamente excluyentes (personal\_rescate, personal\_construccion, victima, conductor), cada una con subtipos adicionales:

    \begin{itemize}[leftmargin=*,itemsep=1pt]
    \item \texttt{personal\_rescate}: medico, rescatista, asistente (3 subtipos)
    \item \texttt{personal\_construccion}: tecnico, obrero, ingeniero, operador (4 subtipos)
    \item \texttt{victima}: victima\_grave, victima\_estable, victima\_no\_afectada (3 subtipos)
    \item \texttt{conductor}: tipo simple sin subtipos
    \end{itemize}

    \textbf{Validación de cardinalidades}: Todas las relaciones 1:1 están implementadas como predicados funcionales que garantizan unicidad (e.g., una víctima solo puede estar en una locación). Las relaciones N:N (como \texttt{adyacente}) permiten múltiples asociaciones bidireccionales.

    \textbf{Cobertura de estados}: Los 9 conceptos principales tienen atributos completos que cubren:
    \begin{itemize}[leftmargin=*,itemsep=1pt]
    \item \textbf{Ubicación espacial}: Todos los objetos móviles tienen atributo \texttt{ubicacionActual}
    \item \textbf{Estado operativo}: Infraestructura tiene atributos \texttt{estado}, \texttt{operativo}, \texttt{construido}
    \item \textbf{Capacidad}: Unidades tienen \texttt{capacidadPersonas} y \texttt{espacioDisponible}
    \item \textbf{Estado de salud}: Víctimas tienen \texttt{estadoSalud}, \texttt{estabilizado}, \texttt{rescatado}
    \end{itemize}

    \subsubsection*{C.5: Análisis de Atributos por Concepto}

    La Tabla~\ref{tab:atributos_concepto} desglosa la cantidad y naturaleza de atributos por concepto, validando que cada entidad tiene la información necesaria para el proceso de planificación.

    \begin{table}[H]
    \caption{Distribución de atributos por concepto}
    \label{tab:atributos_concepto}
    \centering
    \small
    \begin{tabular}{@{}lrrr@{}}
    \toprule
    \textbf{Concepto} & \textbf{Atributos} & \textbf{Mutables} & \textbf{Inmutables} \\
    \midrule
    Victima & 4 & 2 & 2 \\
    UnidadMovil & 5 & 2 & 3 \\
    Infraestructura & 5 & 1 & 4 \\
    PersonalRescate & 4 & 2 & 2 \\
    PersonalConstruccion & 3 & 2 & 1 \\
    Locacion & 4 & 2 & 2 \\
    Medico & 1 & 1 & 0 \\
    Rescatista & 1 & 1 & 0 \\
    Operador & 1 & 1 & 0 \\
    \midrule
    \textbf{Total} & \textbf{28} & \textbf{14} & \textbf{14} \\
    \bottomrule
    \end{tabular}
    \end{table}

    \textbf{Conclusión de validación}: La ontología cubre exhaustivamente el dominio de rescate multietapa. Los 15 conceptos, 15 relaciones y 47 atributos están completamente mapeados a estructuras PDDL válidas, garantizando trazabilidad semántica desde el modelo conceptual hasta la implementación ejecutable. No se detectaron inconsistencias ni gaps de modelado.

    \subsection*{Anexo D: Especificación de Red Semántica}

    La red semántica implementada en formato Graphviz DOT proporciona una representación visual completa de la ontología del dominio, mostrando todas las relaciones entre conceptos, jerarquías y propiedades.

    \subsubsection*{D.1: Estructura de la Red Semántica}

    La red semántica está organizada en clusters temáticos que agrupan conceptos relacionados:

    \begin{table}[H]
    \caption{Organización de clusters en la red semántica}
    \label{tab:clusters_red}
    \centering
    \small
    \begin{tabular}{@{}llr@{}}
    \toprule
    \textbf{Cluster} & \textbf{Color} & \textbf{Nodos} \\
    \midrule
    Locaciones & Azul claro & 5 \\
    Unidades Móviles & Verde claro & 6 \\
    Personal de Rescate & Amarillo & 4 \\
    Personal de Construcción & Naranja claro & 21 \\
    Víctimas & Rosa & 10 \\
    Infraestructura & Púrpura claro & 3 \\
    Recursos & Gris claro & 8 \\
    Conductores & Verde lima & 6 \\
    \midrule
    ...
    \subsubsection*{D.2: Tipos de Relaciones en la Red}

    La Tabla~\ref{tab:relaciones_red} detalla los tipos de aristas utilizadas en la visualización y su significado semántico.

    \begin{table}[H]
    \caption{Tipos de aristas en la red semántica}
    \label{tab:relaciones_red}
    \centering
    \small
    \begin{tabular}{@{}llll@{}}
    \toprule
    \textbf{Relación} & \textbf{Color} & \textbf{Estilo} & \textbf{Significado} \\
    \midrule
    ubicado\_en & Azul & Sólida & Relación espacial \\
    conecta & Púrpura & Gruesa & Infraestructura conecta locaciones \\
    adyacente & Azul & Punteada & Adyacencia topológica \\
    transporta & Verde & Sólida & Unidad transporta persona/recurso \\
    construye & Naranja & Sólida & Personal construye infraestructura \\
    requiere & Rojo & Punteada & Infraestructura requiere recurso \\
    estabiliza & Rosa & Sólida & Médico estabiliza víctima \\
    opera & Amarillo & Sólida & Conductor opera unidad \\
    rescata & Rosa & Gruesa & Personal rescata víctima \\
    \bottomrule
    \end{tabular}
    \end{table}

    \subsubsection*{D.3: Código DOT de Ejemplo}

    El siguiente fragmento muestra la estructura del archivo \texttt{red\_semantica.dot}:

    \begin{lstlisting}[style=anexostyle,caption={Fragmento de la especificación Graphviz DOT},language=bash]
digraph RedSemanticaRescate {
    rankdir=TB;
    splines=ortho;
    nodesep=0.8;
    ranksep=1.2;

    // Cluster de locaciones
    subgraph cluster_locaciones {
        label="LOCACIONES";
        style=dashed;
        color=blue;

        C0 [fillcolor="#ADD8E6", label="C0\n(Base)"];
        C1 [fillcolor="#ADD8E6", label="C1"];
        C2 [fillcolor="#ADD8E6", label="C2"];
        C3 [fillcolor="#ADD8E6", label="C3"];
        M [fillcolor="#FFE4E1", label="M\n(Mina)"];
    }

    // Relaciones de adyacencia
    C0 -> C1 [label="adyacente", color=blue, style=dashed];
    C1 -> C2 [label="adyacente", color=blue, style=dashed];
    C2 -> C3 [label="adyacente", color=blue, style=dashed];
    C3 -> M [label="adyacente", color=blue, style=dashed];

    // Infraestructura conecta locaciones
    PPF -> C1 [label="conecta_desde", color=purple];
    PPF -> C2 [label="conecta_hacia", color=purple, penwidth=2];
}
    \end{lstlisting}

    \subsubsection*{D.4: Propiedades de Visualización}

    \textbf{Configuración de renderizado}:
    \begin{itemize}[leftmargin=*,itemsep=1pt]
    \item \textbf{Layout}: Top-to-Bottom (rankdir=TB) para flujo jerárquico
    \item \textbf{Splines}: Ortogonales para claridad visual
    \item \textbf{Nodos}: Cajas redondeadas con relleno por categoría
    \item \textbf{Fuente}: Arial, tamaño 9-11 puntos
    \item \textbf{Separación}: nodesep=0.8, ranksep=1.2 para legibilidad
    \end{itemize}

    \textbf{Compilación}: El archivo DOT se compila con el comando:
    \begin{verbatim}
    dot -Tpng red_semantica.dot -o red_semantica.png
    \end{verbatim}

    Generando una imagen PNG de alta resolución que muestra todas las 63 entidades del dominio y sus interrelaciones.

    \subsubsection*{D.5: Validación Visual de la Ontología}

    La red semántica permite validar visualmente:

    \begin{enumerate}[leftmargin=*,itemsep=1pt]
    \item \textbf{Completitud de relaciones}: Todas las entidades tienen al menos una conexión (no hay nodos aislados)
    \item \textbf{Jerarquía de tipos}: Los clusters muestran claramente la agrupación conceptual
    \item \textbf{Flujo de información}: Las relaciones direccionales muestran dependencias de construcción C0→C1→C2→C3→M
    \item \textbf{Cardinalidades}: El grosor de líneas indica relaciones 1:1 (finas) vs 1:N (gruesas)
    \item \textbf{Tipos de relaciones}: Los colores distinguen relaciones espaciales, de transporte, construcción y rescate
    \end{enumerate}

    \textbf{Verificación de consistencia}: La compilación exitosa del archivo DOT sin errores confirma que:
    \begin{itemize}[leftmargin=*,itemsep=1pt]
    \item Todos los nodos referenciados están definidos
    \item No hay ciclos problemáticos en relaciones funcionales
    \item Las etiquetas son únicas dentro de cada cluster
    \item Los colores y estilos son consistentes por tipo de relación
    \end{itemize}

    \subsubsection*{D.6: Estadísticas de la Red}

    \begin{table}[H]
    \caption{Métricas de la red semántica}
    ...
    \midrule
    Total de nodos & 63 \\
    Total de aristas direccionales & 87 \\
    Clusters definidos & 8 \\
    Relaciones de tipo ``en'' (ubicación) & 59 \\
    Relaciones de tipo ``conecta'' & 3 \\
    Relaciones de tipo ``adyacente'' & 8 \\
    Relaciones de tipo ``transporta'' & 12 \\
    Relaciones de tipo ``construye'' & 3 \\
    Profundidad máxima (C0→M) & 4 niveles \\
    Grado promedio por nodo & 2.76 \\
    \bottomrule
    \end{tabular}
    \end{table}

    \textbf{Conclusión}: La red semántica implementada en Graphviz proporciona una representación visual completa y validada de la ontología. La estructura en clusters facilita la comprensión del dominio y la verificación de relaciones. El archivo DOT de 288 líneas documenta exhaustivamente todos los conceptos, instancias y relaciones del sistema de planificación de rescate multietapa.
